...
- Task 9 trả tối đa một SearchResult trên mỗi URL nguồn đã chuẩn hóa. Nếu nhiều chunk cùng nguồn được truy xuất, `content` chứa các đoạn đó nối với nhau; `id` và `chunk_index` nhận diện chunk xếp hạng cao nhất làm đại diện.
- Với truy vấn nêu số Điều hoặc số Mẫu cụ thể, Task 9 chỉ giữ tài liệu có đúng số đó; `Mẫu số 1` không khớp `Mẫu số 01`.
