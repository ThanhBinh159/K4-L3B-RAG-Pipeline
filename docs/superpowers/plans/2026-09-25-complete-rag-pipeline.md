...
The PageIndex branch has no live key in this workspace. The earlier A/B report uses OpenRouter embeddings with extractive answers. The current `.env` uses Ollama `bge-m3:latest` embeddings: 399 chunks indexed. All 15 golden questions ran through the configured Gemini gateway with verified source quotations; results and proxy metrics are in `group_project/evaluation/RESULT_ONLINE.md` and `online_results.json`. The complete test suite passed (39 tests).
