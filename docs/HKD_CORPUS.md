...
- `group_project/evaluation/golden_dataset.json` có 15 câu hỏi với đáp án và đoạn bằng chứng từ corpus. `group_project/evaluation/RESULT.md` ghi kết quả A/B offline với OpenRouter embedding và câu trả lời trích xuất; `group_project/evaluation/RESULT_ONLINE.md` ghi kết quả chạy Ollama `bge-m3:latest` cùng Gemini gateway trên cùng 15 câu. Bốn metric là proxy so khớp văn bản/nhãn nguồn, không phải đánh giá RAGAS hoặc xác nhận pháp lý bởi chuyên gia. Phản hồi đầy đủ từng câu ở `group_project/evaluation/online_results.json` được tạo cục bộ và bị loại khỏi Git.
